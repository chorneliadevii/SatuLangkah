...
@WebServlet(name = "editProduk", urlPatterns = {"/editProduk"})
public class editProduk extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       response.setContentType("text/html;charset=UTF-8");
        ...
        if (request.getParameter("submit").equals("Hapus")) {
            menu.hapusProduk(idProduk);
            response.sendRedirect("./tampilProdukAdmin");
        } else {
            lokalProdukTampil tampil = menu.getProduk(idProduk);
            session.setAttribute("produk", tampil);
            RequestDispatcher rd = request.getRequestDispatcher("/editLokalProduk.jsp");
